 ...
 */
 ...
 function isInWithin(target, elem) {
   var isIn = target[0];
   while(elem) {
     if(isIn === elem) return true;
...
$.TokenList = function (input, url_or_data, settings) {
            ...
            switch(event.keyCode) {
                ...
                case KEY.COMMA:
                  if(selected_dropdown_item) {
                    if(!isInWithin($('div[data-infoField="taskAssignee"]'), this)) {
    ...
    function delete_token (token) {
        ...
        if(settings.tokenLimit !== null) {
            if(input_box.val()) input_box.show().focus();
        }
            ...
            if(settings.url) {
                ...
                var results = $.grep(settings.local_data, function (row) {
                  ...
                  for(var x=0; x<temp.length; x++) {
                    if(row[temp[x]].toLowerCase().indexOf(query.toLowerCase()) > -1) {
                      return row[temp[x]];
                    }
                  }
                });
